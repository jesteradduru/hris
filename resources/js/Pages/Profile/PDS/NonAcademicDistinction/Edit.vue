        ...
        <div class="row">
          ...
          <div class="col-12">
            <div class="d-flex gap-2">
              ...
              <button class="btn btn-success" :disabled="submitForm.processing" type="submit"><Spinner :processing="form.processing" />  Update</button>
            </div>
          </div>
        </div>
      </form>
...
const form = useForm({
  _method: 'put',
  title: props.distinction.title,
  office: props.distinction.office,
  date_awarded: props.distinction.date_awarded ? moment(props.distinction.date_awarded).format('Y-M-D') : null,
  documents: [],
})
  
const submitForm = () => {
  form.post(route('profile.pds.non_academic_distinctions.update', {non_academic_distinction: props.distinction.id}), {
    preserveScroll: true,
  })
}
