...
class LonglistedApplicants implements FromCollection, ShouldAutoSize, WithMapping, WithHeadings, WithStyles, WithColumnWidths, WithDefaultStyles, WithCustomStartCell, WithDrawings
{
    /**
    ...
    */
    ...
    public function collection() {
        if($this->job_posting){
            ...
            $job_results = JobApplicationResults::where('job_posting_id', $this->job_posting)->where('phase', 'INITIAL_SCREENING')->with(
                [
                    'result' => [
            ...
            $job_posting_result = JobApplicationResults::where('phase', 'INITIAL_SCREENING')->with(
                [
                    'result' => [
                        'user' => ['personal_information', 'page_four_questions']
                    ],
            ...
            foreach($job_posting_result as $result){
                foreach($result->result as $application_result){
                    if($application_result->result === 'QUALIFIED'){
                        $applicant = null;
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet->setCellValue('A4', 'List of Longlisted Applicants');
